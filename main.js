...
async function checkWeather(lat, long) {
    ...
    currentDate.innerHTML = nextDay.toString().slice(0, 3);
    city.innerHTML = data.city.name;
    temp.innerHTML = Math.round(data.list[currentDayIndex * 8 + 4].main.temp) + "°F";
    humidity.innerHTML = data.list[currentDayIndex * 8 + 4].main.humidity + "%";
    wind.innerHTML = Math.round(data.list[currentDayIndex * 8 + 4].wind.speed) + " mph";
